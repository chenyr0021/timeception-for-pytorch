...
class Unit3D(nn.Module):
    ...
    def forward(self, x):
        ...
        pad = list([pad_w_f, pad_w_b, pad_h_f, pad_h_b, pad_t_f, pad_t_b])
        # print x.size()
        # print(pad)
        x = F.pad(x, pad)
        # print x.size()
...
class InceptionI3d(nn.Module):
    ...
    def __init__(self, num_classes=157, spatial_squeeze=True, final_endpoint='Logits', name='inception_i3d', in_channels=3, dropout_keep_prob=0.5):
        ...
        if self._final_endpoint == end_point: self.build(); return
...
#      model = InceptionI3d(final_endpoint='Mixed_5c')
#      # model = Unit3D(in_channels=3, output_channels=64, kernel_shape=[7, 7, 7], stride=(2, 2, 2), padding=3, name='1')
#
#      tensor = torch.tensor(np.zeros((1,3,256,224,224)), dtype=torch.float)
#      model.replace_logits(157)
#      out = model.extract_features(tensor)
#      print(out.shape)
