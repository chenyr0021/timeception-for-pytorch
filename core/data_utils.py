...
class DatasetCharades(torch.utils.data.Dataset):
    def __init__(self, batch_size, n_classes, feature_dim, feature_name, is_training, is_shuffle=True):
        ...
        # convert relative to root paths
        feats_path = ['%s/%s/%s.pkl' % (root_path, feature_name, p.decode()) for p in video_names]
        # check if all the features are exist
        y_exist_ind = []
        for i, path in enumerate(feats_path):
            if os.path.exists(path):
                y_exist_ind.append(i)
        feats_path = np.array([feats_path[i] for i in y_exist_ind])
        y = np.concatenate([y[i][np.newaxis, :] for i in y_exist_ind], axis=0)
