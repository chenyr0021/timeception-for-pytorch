...
def train_tco():
    ...
    model_name = '%s' % (config.cfg.MODEL.NAME)
    ...
    # logger.info(pytorch_utils.summary(model, model.module._input_shape[1:], batch_size=1, device='cuda'))

    # save the model
    model_saver = pytorch_utils.ModelSaver(model, dataset_name, model_name)
    state_dict = model_saver.load()
    if state_dict:
    ...
    for idx_epoch in range(n_epochs):
        ...
        for idx_batch, (x, y_true) in enumerate(loader_tr):
            ...
            duration = tt2 - tt1
            print(acc_b_tr)
            print('\r%04ds - epoch: %02d/%02d, batch [tr]: %02d/%02d, loss: %0.2f, %s: %0.2f' %
                (duration, epoch_num, n_epochs, batch_num, n_batches_tr, loss_b_tr, metric_fn_name, acc_b_tr))
        ...
        for idx_batch, (x, y_true) in enumerate(loader_te):
            ...
            tt2 = time.time()
            duration = tt2 - tt1
            sys.stdout.write('\r%04ds - epoch: %02d/%02d, batch [te]: %02d/%02d, loss, %s: %0.2f, %0.2f ' %
                             (duration, epoch_num, n_epochs, batch_num, n_batches_te, metric_fn_name, loss_b_te, acc_b_te))
...
def inference():
    input = utils.pkl_load('./data/charades/features_i3d_pytorch_charades_rgb_mixed_5c_32f/06OKE.pkl')
    input = np.transpose(input, (3,0,1,2))
    input = input[np.newaxis, :]
    input = torch.from_numpy(input)
    label_vector = utils.pkl_load('./data/charades/annotation/video_annotation.pkl')[1][44]
    f = open('./i3d_pytorch/Charades/Charades_v1_classes.txt')
    charades_classes = f.readlines()
    f.close()
    classes = [charades_classes[i] for i in np.where(label_vector==1)[0]]
    print("label: ", classes)
    model = Model()
    model.eval()
    pred = model(input).squeeze()
    pred = pred.detach().numpy()
    indies = np.argsort(pred)
    # sorted_pred = [pred[i] for i in indies]
    # sorted_label = [label_vector[i] for i in indies]
    for i in range(len(pred)):
        print(label_vector[i], '  ', pred[i])
...
def __define_loader(is_training):
    ...
    # size and name of feature. example: features_i3d_pytorch_charades_rgb_mixed_5c_32f
...
def __define_timeception_model(device):
    ...
    #     print("Using data parallel. GPU*%d" % torch.cuda.device_count())
    model.to(device)
...
class Model(Module):
    ...
    def __init__(self):
        ...
        n_classes = config.cfg.MODEL.N_CLASSES
        is_dilated = config.cfg.MODEL.MULTISCALE_TYPE == 'dl'
